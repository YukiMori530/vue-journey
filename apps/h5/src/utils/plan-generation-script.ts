...
export function buildPlanGenerationSteps(destination: string, days: number, fullPrompt: string): PlanLogStep[] {
  ...
  return [
    ...
    { kind: 'search', text: `正在搜索小红书和全网信息：${destination}${days}日游行程攻略 ${year}`, delayMs: 900 },
    { kind: 'search', text: `正在搜索小红书和全网信息：${destination}${hints.spots[0]}开放时间门票${year}`, delayMs: 1100 },
    { kind: 'search', text: `正在搜索小红书和全网信息：${destination}旅游注意事项最佳路线`, delayMs: 1000 },
    { kind: 'location', text: `正在搜索${destination}的地点：必去景点（${hints.spots.slice(0, 3).join('、')}等）`, delayMs: 1200 },
    { kind: 'location', text: `正在搜索${destination}的地点：本地美食餐厅（${hints.foods.join('、')}）`, delayMs: 900 },
    { kind: 'location', text: `正在搜索${destination}的地点：夜市与特色街区`, delayMs: 800 },
    {
      kind: 'result',
    ...
    { kind: 'intro', text: '信息已收集，现在计算路线顺序。', delayMs: 700 },
    { kind: 'intro', text: '正在计算路线顺序…', delayMs: 1500 },
    { kind: 'check', text: '🚀 行程规划基本完成，正在第 1 次进行合理性验证…', delayMs: 1200 },
    { kind: 'adjust', text: '检查完了，有一些小的优化空间，正在继续调整。', delayMs: 900 },
    {
      kind: 'adjust',
      text: `验证通过，正在优化：1) 第 1 天路线顺序；2) 第 2 天精简串联；3) 第 3 天补充${hints.spots[3] ?? '特色景点'}。`,
      delayMs: 1100,
    },
    { kind: 'location', text: `正在查找地点信息：${hints.spots.slice(2, 4).join('、')}`, delayMs: 1000 },
    { kind: 'check', text: `✅ 已帮你定位到 ${hints.spots.slice(2, 4).join('、')}。`, delayMs: 800 },
    { kind: 'intro', text: '好的，地点已补充…现在输出最终行程。', delayMs: 700 },
    { kind: 'summary', text: fullPrompt.replace(/^帮我规划/, '为你规划'), delayMs: 600 },
  ]
}
