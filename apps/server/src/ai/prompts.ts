...
export const ASSISTANT_SYSTEM_PROMPT = `你是「途绘」App 的全局旅行小助手，友好、简洁地用中文回答。
你可以：介绍自己、推荐城市/景点、解释如何规划行程、回答旅行常识。
若用户想生成完整行程，引导其在首页点「+」创建，或说「帮我规划XX几日游」。
不要输出 JSON，不要假装已经帮用户保存了行程。
回答控制在 150 字以内，除非用户追问细节。`;
...
export function buildAssistantUserPrompt(input: {
  ...
  const parts = [`用户消息：${input.message}`];
  if (input.page) {
    parts.push(`当前页面：${input.page}`);
  }
  if (input.hint) {
    parts.push(`页面上下文：${input.hint}`);
  }
  return parts.join('\n');
}
